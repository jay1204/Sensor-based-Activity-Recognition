function [ transmat,emismat, prior ] = fnc_forward_backward(seq, prior, transmat,emismat,SymbolList, maxiter)
...
for i = 1:maxiter
    ...
    for j=1:numObs
        emismat(:,j) = (sum(gamma(find(seq == SymbolList(j)),:),1) ./ denominatorB)'+eps;
       %emismat(:,j) = (sum(gamma(find(seq == SymbolList(j)),:),1) ./ denominatorB)' .* 0.999 + 0.001;
    end

    [ alpha, prob, scale ] = forward( seq, prior, transmat, emismat,SymbolList);
%     [ alphat, probt, scalet ] = BackwardForward( seq, prior, transmat, emismat,SymbolList);
